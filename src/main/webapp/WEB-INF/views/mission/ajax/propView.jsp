...
<form method="post">
	...
	<input type="datetime-local" name="propTimestamp" required placeholder="propTimestamp" />
		...
		fetch(url, options)
			.then(resp=>{
				if(resp.ok){
					...
					thorw new Error(`\${url}처리 중 에러 발생`, {cause:resp});
				}
			}).then(jsonObj=>{
				console.log(jsonObj);
...
// 		let formData = new FormData(form);
// 		let urlSearchParams = new URLSearchParams(forData);
// 		ajax(urlSearchParams)
// 	})
	
// 	let ajax = $.ajax({
// 		method : 'post',
// 		url : '/mission/ajax/propView',
